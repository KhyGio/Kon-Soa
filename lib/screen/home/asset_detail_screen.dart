...
class _PasswordDetailScreenState extends State<PasswordDetailScreen> {
  ...
  Future<void> _copyPassword() async {
    ...
    if (_decryptedPassword != null) {
      ...
      try {
        password = await _repository.getDecryptedPassword(widget.model.id);
      } catch (error) {
        if (!mounted) {
          return;
        }
  ...
  Future<void> _delete() async {
    ...
    try {
      await _repository.deletePassword(widget.model.id);
  ...
  Widget _passwordField() {
    final displayValue = _showPassword
        ? (_decryptedPassword ?? '')
        : '••••••••';
        ...
        children: [
          ...
          Row(
            children: [
              ...
              IconButton(
                onPressed: _loadingPassword ? null : _copyPassword,
                icon: const Icon(
                  Icons.copy_outlined,
                  color: AppTheme.textSecondary,
                  size: 18,
